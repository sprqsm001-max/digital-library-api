/*
  ...
  Updated to handle EPUB metadata extraction and Many-to-Many Authors.
*/
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Add listener for file upload to trigger auto-fill
  const fileInput = document.getElementById('book-file-upload');
  if (fileInput) {
    fileInput.addEventListener('change', handleFileAutoFill);
  }
});

async function handleFileAutoFill(e) {
  const file = e.target.files[0];
  if (!file || !file.name.toLowerCase().endswith('.epub')) return;

  try {
    showToast('Extracting metadata from EPUB...');
    const uploadRes = await API.uploadBookFile(file);

    // Store temporary upload results in form dataset
    const form = document.getElementById('add-book-form');
    form.dataset.tempFileUrl = uploadRes.url;
    form.dataset.tempFileType = uploadRes.file_type;
    form.dataset.tempFileSize = uploadRes.file_size_kb;

    if (uploadRes.metadata) {
      const meta = uploadRes.metadata;
      if (meta.title) document.getElementById('book-title').value = meta.title;
      if (meta.authors && meta.authors.length > 0) {
        document.getElementById('book-author').value = meta.authors.join(', ');
      }
      if (meta.publisher) document.getElementById('book-publisher').value = meta.publisher;
      if (meta.publication_year) document.getElementById('book-year').value = meta.publication_year;
      if (meta.isbn) document.getElementById('book-isbn').value = meta.isbn;
      if (meta.description) document.getElementById('book-desc').value = meta.description;

      showToast('Metadata auto-filled from EPUB!');
    }
  } catch (err) {
    console.error('Auto-fill error:', err);
    showToast('Could not extract metadata, please enter manually.', 'info');
  }
}
...
async function submitAddBook(e) {
  ...
  let file_url = form.dataset.tempFileUrl || null;
  let file_type = form.dataset.tempFileType || 'epub';
  let file_size_kb = parseInt(document.getElementById('book-size').value) || parseInt(form.dataset.tempFileSize) || 0;
  let cover_image_url = document.getElementById('book-cover-url').value.trim() || null;

  try {
    // 1. Handle Book File Upload (if not already uploaded via auto-fill)
    if (!file_url && bookFileInput.files.length > 0) {
    ...
    const authorNames = authorInput.split(',').map(name => name.trim()).filter(name => name !== "");

    const bookData = {
      title: document.getElementById('book-title').value.trim(),
      author_names: authorNames, // New Many-to-Many field
      ...
      file_url: file_url || "https://example.com/pending", // Backend requires file_url
      file_type: file_type,
    ...
    form.reset();
    delete form.dataset.tempFileUrl;
    delete form.dataset.tempFileType;
    delete form.dataset.tempFileSize;
    toggleAddBookForm();
